...
class GraspNetDataset(Dataset):
    ...
    def get_data_label(self, index):
        ...
        if self.remove_outlier:
            ...
            mask = depth_mask
        cloud_masked = cloud[mask]         # get the object in table
        color_masked = color[mask]
        seg_masked = seg[mask]
...
def load_grasp_labels(root):
    ...
    for i, obj_name in enumerate(tqdm(obj_names, desc='Loading grasping labels...')):
        ...
        grasp_labels[i + 1] = (label['points'].astype(np.float32), label['offsets'].astype(np.float32),
                                score['new_scores'].astype(np.float32))
...
if __name__ == "__main__":
    ...
    object_label = end_points['objectness_label']
    ob_pose = end_points['object_poses_list']
    grasp_point = end_points['grasp_points_list']
    grasp_offset = end_points['grasp_offsets_list']
    grasp_label = end_points['grasp_labels_list']
    grasp_tolerance = end_points['grasp_tolerance_list']
